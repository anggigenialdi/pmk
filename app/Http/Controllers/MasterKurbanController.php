...
class MasterKurbanController extends Controller
{
    ...
    public function masterKurbanPost(Request $request)
    {
        ...
        $duplicate = Kurban::where('id_kecamatan', $request->input('kecamatan'))->where('id_kelurahan', $request->input('kelurahan'))->first();

        if ($duplicate) {
            return back()->with('warning', ' Data Sudah Ada');
        } else {
    ...
    public function masterKurbanDetail()
    {
        $datas = Kurban::orderBy('id', 'desc')->get();
